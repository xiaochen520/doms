...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [10, 20, 30, 40, 50,100,500,1000],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                    <div class='l-grid-popup' style='height:200px;'><table cellpadding='0' cellspacing='0'><tbody></tbody></table></div>");
            ...
            if (column.totalSummary)
            {
                ...
                else if (column.totalSummary.type)
                {
                    var types = column.totalSummary.type.split(',');
                    	if(count!=0){
	                    	if (isExist('sum'))
	                            totalsummaryArr.push("<div style='text-align: center; color: green;'>" + sum.toFixed(2) + "</div>");
	                    	if (isExist('avg'))
	                            totalsummaryArr.push("<div style='text-align: center; color: blue;'>" + avg.toFixed(2) + "</div>");
                    	}
	                    if (isExist('count'))
                            totalsummaryArr.push("<div style='text-align: center; color: green;'>合计：</div><div style='text-align: center; color: blue;'>平均：</div>");
                        if (isExist('max'))
