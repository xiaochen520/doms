...
<head><title>SAGD井日报数据维护</title>
    ...
    <script type="text/javascript">
        ...
        $(function () {
			...
			seachSelectData();
                jQuery.ajax({
					type : 'post',
					url : 'sagddrpd_SagdRPDpageInit.action?arg=wh',
					async : false,
      ...
      function setGHData(oilationnameid,proData) {
		...
		var oilationnameText=[{ id: 1 , text: '' }];
		var ghmcText=[{ id: 1 , text: '' }];
		var wellText=[{ id: 1 , text: '' }];
		jQuery.ajax({
			type : 'post',
			url : 'sagd_cascadeInit1.action',
			success : function(jsondata) {
			var dataObj = $.parseJSON(jsondata);
				$.each(dataObj, function(key,val){
					if (key == 'oilationnameText'){
						oilationnameText = val;
					}
					if (key == 'ghmcText'){
						ghmcText = val;
					}
				...
				setInitData(oilationnameText,ghmcText,wellText);
			}
		});
	}
	...
	function setInitData(oilationnameText,ghmcText,wellText) {
...
<!-- 		               <td align="right" class="l-table-edit-td" style="width:60px">区块：</td>
	                 ...
	                 <td align="right" class="l-table-edit-td" style="width:60px">采油站：</td>
						...
						<td style="padding-left: 20px;"><a href="javascript:onSubmit()" class="l-button" style="width:80px">查询</a></td>
 						...
 						<td style="padding-left: 20px;"><a href="javascript:dayreport()" class="l-button" id="dayreport" style="width:80px;display:none;" title="8点后才可以生成当天的日报; " >生成日报</a></td> 
