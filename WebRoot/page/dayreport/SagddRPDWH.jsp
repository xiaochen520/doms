    ...
    <script type="text/javascript">
        ...
        $(function () {
			...
			seachSelectData();
        		 ...
        		 $("#oilationname").ligerComboBox({
        			...
        			liger.get("ghmc").setValue('');	        		
                		if ($("#oilationnameid").val() != 1 && data != null && typeof(data)!="undefined" && data != '') {
        					...
        					//setBZData($("#oilationnameid").val(),proData);         					
        				}
         				else {
        					setGHData();
        					//setBZData();
        		...
        		$("#BZHF").ligerComboBox({
                    ...
                    onSuccess:function (data){
                    	if (data != null && typeof(data)!="undefined" && data != ''){
        					setWellInitData($("#bzhfid").val(),proData);        					
        					}        				  
        			} 
        		});
			...
			jQuery.ajax({
        		...
        		//关联只显示SGAD井下的管汇
			function setBZData(text,proData) {
  			//alert(oilationnameid);
			jQuery.ajax({
				type : 'post',
				...
				success : function(jsondata) {
					if (jsondata != null && typeof(jsondata)!="undefined" && jsondata!=0){
						liger.get("BZHF").setData(eval('(' + jsondata + ')'));
					}
					else{
						liger.get("BZHF").setData(proData);
					}
				},
				error : function(jsondata) {
					alert("请求数据失败，请重新查询");
