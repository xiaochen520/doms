           	...
           	//获取报表及功能按钮ｊｓ
                jQuery.ajax({
   				...
   				jQuery("#edituser").css('height',document.body.clientHeight * 0.55);
	                ...
	                { display: 'P井注汽流量计', width: 80,type:"int",id: 'P_FLOWMETER', name: 'P_FLOWMETER',
	                    editor: { type: 'select', data: P_FLOWMETERData, valueField: 'P_FLOWMETER',
	                    onSelected: function (value,text){
	                    //	if(value){
	                    	//	alert(value+"---------"+text);
	                    	//	var ifowdata = new Array();
	                    	//	if(value == 1){
	                    		//flagNum=1;
	                    		//	ifowdata.push({ P_FLOWMETER: 2, text: '2号流量计'});
	                    		//}else if(value == 2){
	                    		//flagNum=2;
	                    		//	ifowdata.push({ P_FLOWMETER: 1, text: '1号流量计'});
	                    		//}else if(value == 3){
	                    	//	flagNum=3;
	                    		//	ifowdata.push([]);
	                    		//}else{
	                    		//	ifowdata.push({ I_FLOWMETER: 1, text: '1号流量计' }, { I_FLOWMETER: 2, text: '2号流量计'}, { I_FLOWMETER: 3, text: '双流量计'});
	                    		//}
	                    		//P_FLOWMETERData
	                    		//var cmbiflowmanger = $("td[id^=jkmaingrid][id$=I_FLOWMETER] input[type=text]");
	                    		//.ligerGetComboBoxManager()
	                    		//alert(JSON2.stringify(cmbiflowmanger));
	                    		//if(cmbiflowmanger != null){
	                    			//cmbiflowmanger.setData(ifowdata);
	                    		//}
	                    ...
	                    {
	                    	//alert(JSON2.stringify(item));
	                        //if (parseInt(item.P_FLOWMETER) == 1) {
	                        	//P_FLOWMETERData = [{ P_FLOWMETER: 2, text: '2号流量计'}];
	                        	//return '1号流量计';
	                        //}
	                        if (parseInt(flagNum) == 1) {
	                        //alert(flagNum);
	                        	//P_FLOWMETERData = [{ P_FLOWMETER: 2, text: '2号流量计'}];
	                        	return '1号流量计';
	                        }
	                        else if (parseInt(item.P_FLOWMETER) == 2) 
	                        {
	                        	//P_FLOWMETERData = [{ P_FLOWMETER: 1, text: '1号流量计'}];
	                       		return '2号流量计';	
	                        }
	                        else if (parseInt(item.P_FLOWMETER) == 3){
	                        	return '双流量计';
	                        	//P_FLOWMETERData = [{}];
	                        } 
	                        else {
	               ...
	               { display: 'I井注汽流量计', width: 80,type:"int",id: 'I_FLOWMETER', name: 'I_FLOWMETER',
	                    editor: { type: 'select', data: I_FLOWMETERData, valueField: 'I_FLOWMETER' },
	                    render: function (item)
	                    {
	                    	
	                        if (parseInt(item.I_FLOWMETER) == 1){
	                        	return '1号流量计';
	                        } 
	                        else if (parseInt(item.I_FLOWMETER) == 2){
	                        	return '2号流量计';
	                        } 
	                        else if (parseInt(item.I_FLOWMETER) == 3){
	                        	return '双流量计';
	                        } 
	                        else{
	                        	return '';
          ...
          switch (item.icon) {
              ...
              case "delete":
              	  if (rows.length == 0) { 
    					...
    					if (this.VALID_DATE+'' !=myDate.getFullYear()+'-'+myDate.getMonth() + 1+'-'+myDate.getDate()) {
              ...
              case "save":
            	...
            	var tb = JSON2.stringify(jkmanager.getData());
					...
					//var date3  = myDate().Format("yyyy-MM-dd");
					//alert(date3);
					//if(tabledate == date3){
						//alert(tabledate == date3);
					//}
					$(rows).each(function (){
						//alert(this.VALID_DATE);
						//var myDate2 = new Date(this.VALID_DATE);
						if (this.VALID_DATE+'' !=myDate.getFullYear()+'-'+myDate.getMonth() + 1+'-'+myDate.getDate()) {
...
<div style="width:570px;  margin:10px; float:left; border:1px solid #ccc;   ">
	<table border="0">
	<tr>
	...
	<tr>
		<td>
			<div style="width: 360px;height: 210px;border:1px solid #ccc;" >
		         <div id="jkmaingrid" style="width: 370px;height: 210px;"></div> 
		    </div>
		</td>
		<td>
			 <div style="margin:4px;float:left;" class="middle">
