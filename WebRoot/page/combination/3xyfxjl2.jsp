       ...
       function onSubmit()
        {
        	 jQuery.ajax({
					type : 'post',
					url : 'xyfxsc3_searchFxjl3.action?txtDate='+$("#txtDate").val(),
					success : function(data) { 
					if (data != null && typeof(data)!="undefined"&& data!=""){
							var outData = eval('(' + data + ')');
							//alert(data)
...
//									    var U1S1SS = outData.JSONDATA.U1S1SS;									    
						    	...
						    	$("#tableHtml").html('');
					     		 ...
					     		 "collapse;table-layout:fixed;width:1150pt'>"+
					     		...
					     		" </tr>"+	
 										...
 										" <td class=xl65>值班人：</td>"+
											"<td colspan=3 class=xl65><input id='ZBR' type='text' style='background:transparent;border:0px solid;font-size: 12pt;text-align:center' disabled='disabled'/></td> "+							     		
							     		" <td  class=xl65>审核：</td>"+
											"<td colspan=3  class=xl65><input id='SHR' type='text' style='background:transparent;border:0px solid;font-size: 12pt;text-align:center' disabled='disabled' value='"+SHR+"'/></td> "+							     		
							     		" <td  class=xl65>日期：</td>"+
										" <td colspan=4 class=xl65><input id='RQ' type='text'  style='background:transparent;border:0px solid;font-size: 12pt;text-align:center' disabled='disabled' value='"+RQ+"'/></td> "+							     		
							     		 "</tr>"+			
							     		...
							     		" <td bgColor=#CCCCFF class=xl65 style='border-left:none'>盘库数据</td>"+
							     		 " <td bgColor=#CCCCFF colspan=9 class=xl65 style='border-left:none'>备注</td>"+
							     		"</tr>"+							     		
							     				"<tr height=24 style='mso-height-source:userset;height:18.0pt'>"+
							     		 ...
							     		 " <td bgColor=#CCCCFF colspan=9 class=xl65 style='border-left:none'>分线仪表及自动化采集情况</td>"+							     		  
							     	...
							     	for ( var i = 0; i < U2OILS.length; i++) {
							     		...
							     		tableval += "<tr height=24 style='mso-height-source:userset;height:18.0pt'>"+
								     		   "<td style='display: none'><input id='rpd_xyl_sc_messure_id' type='hidden' value='"+U2OILS[i].rpd_xyl_sc_messure_id+"'></td>"+							
								     		 "<td id='report_date' height=24 class=xl65  style='height:18.0pt;border-top:none'>"+U2OILS[i].report_date+"</td>"+	
								     		  "<td class=xl65  style='border-bottom:.5pt solid black;border-top:none'><input id='jhgye'  type='text'  "+disStr+"    onkeyup='checkData(this)' style='background:transparent;border:0px solid;width:70px;line-height:25px;height:18.0px;font-size: 10pt;text-align:center;' value="+U2OILS[i].jhgye+"></td>"+
								     		  ...
								     		  "<td class=xl65  style='border-top:none;border-left:none'><input id='pk'  type='text'  "+disStr+"    onkeyup='checkData(this)' style='background:transparent;border:0px solid;width:70px;line-height:25px;height:18.0px;font-size: 10pt;text-align:center;' value="+U2OILS[i].pk+"></td>"+
								     		  "<td class=xl65 colspan=9 style='border-top:none;border-left:none'><input id='remark'  type='text'  "+disStr+"    onkeyup='checkData(this)' style='background:transparent;border:0px solid;width:1150px;line-height:25px;height:18.0px;font-size: 10pt;text-align:left;' value="+U2OILS[i].remark+"></td>"+
											"</tr>";
						     	 	 }	
					     	 	 } 
          ...
          function onSave()
        	...
        	var	RQ = $("#RQ").val();    		
          	if (SHR != null && typeof(SHR)!="undefined" && SHR != ""){
          		...
          		var firstArr = [];
              	var firstStr = "";																												
             		firstArr.push($("#rpd_xyl_sc_messure_id").val());
             		firstArr.push($("#report_date").val());
             		firstArr.push($("#jhgye").val());
             		firstArr.push($("#jhgyou").val());
             		firstArr.push($("#pk").val());
                ...
                var myday = new Date().Format("yyyy-MM-dd");
             	if (modifyDataFlag(myday)) {
          		 jQuery.ajax({
  					type : 'post',
  					url : 'xyfxsc3_update.action?nowdata='+parseInt(Math.random()*100000),
...
//         		if (chekAduitByDate($("#txtDate").val())) {
         		 jQuery.ajax({
 					type : 'post',
 					url : 'xyfxsc3_onAudit.action?nowdata='+parseInt(Math.random()*100000),
