...
public class SHYSCLRBServiceImpl implements SHYSCLRBService{
	private SHYSCLRBDao shysclrbDao;
	private CommonDao commonDao;


	
	public void setShysclrbDao(SHYSCLRBDao shysclrbDao) {
		this.shysclrbDao = shysclrbDao;
	}
	...
	public JSONArray searchDataSet(String txtDate) throws Exception {
		...
		String sql = "select T.RPD_SHYSCL_T_ID,to_CHAR(T.BBSJ,'YYYY-MM-DD HH24:MI:SS'),RHQZCSL,  CYQZCSL,  ZYBZCSL,  LT_30101A,  "+
					 "LT_30101B,  LT_30102A,	LT_30102B,	LT_30301,	LT_30302,	ZYBYL1,	ZYBYL2,ZYBYL3,"+
					 "YSKQYL1	,YSKQYL2,YSKQYL3,HARDNESS,OXYGEN_BEARING,CHLORIDE,BASICITY"+
					 " from PC_RPD_SHYSCL_T T where 1=1";
		...
		if(disData !=null && disData.size()>0){
			...
			obj = new JSONObject();
					 ...
					 if(dataflg == 1){
							obj.put("RPD_SHYSCL_T_ID", CommonsUtil.getClearNullData(tableData[0]));
							...
							obj.put("ZYBYL3",CommonsUtil.getNotTwoData1(tableData[13]));
							obj.put("YSKQYL1",CommonsUtil.getNotTwoData1(tableData[14]));
							
							obj.put("YSKQYL2",CommonsUtil.getNotTwoData1(tableData[15]));
							obj.put("YSKQYL3",CommonsUtil.getNotTwoData1(tableData[16]));
							obj.put("HARDNESS",CommonsUtil.getNotOneData(tableData[17]));
							obj.put("OXYGEN_BEARING",CommonsUtil.getNotTwoData1(tableData[18]));
							obj.put("CHLORIDE",CommonsUtil.getNotOneData(tableData[19]));
							obj.put("BASICITY",CommonsUtil.getNotOneData(tableData[20]));
						
						
					 	}else{
							
							obj.put("RPD_SHYSCL_T_ID", "");
							obj.put("BBSJ", data.substring(11, 16));
	...
	public boolean updateData(PcRpdShysclT wt) throws Exception {
		...
		return shysclrbDao.saveData(wt);
	}

	@Override
	public List<PcRpdShysclT> SreachCheckOn(String rpdid, String bbsj)
			throws Exception {
		String hql = "  from PcRpdShysclT  a  where  1=1 ";
		if(rpdid !=null &&  !"".equals(rpdid)){
			hql +=" and a.rpdShysclTId = '"+rpdid+"'";
		}
		List<?> list =null;
		list = commonDao.searchClassQuery(hql);
		return (List<PcRpdShysclT>) list;
	}
