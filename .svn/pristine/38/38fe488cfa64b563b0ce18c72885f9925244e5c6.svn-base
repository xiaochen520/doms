...
<head>
	...
	function onExport() {
		...
		var url = "u2szjc_exportU2_SZJC.action?txtDate="+encodeURIComponent(txtDate) + '&reportName='+encodeURIComponent('二号稠油联合站原油系统日报');
       ...
       function itemclick(item) {
		...
		function f_onBeforeEdit(e)
		{
			...
			if(authoruty  == 1){
	        	...
	        	if(e.record.RPD_U2_WATER_QUALITY_ID != "空行"){
					...
					if(e.record.REPORT_DATE == datestr) {
						...
						if (columnname != '' && columnname != null && typeof(this.VERIFIER)!=columnname &&
			        		...
			        		if(textValue.indexOf("span") > 0 ){
				        			...
				        			grid.updateCell(e.column.columnname, textValue, e.record);
				        			//alert(1);
				        		}
				        	}
						...
						$.ligerDialog.error("只能修改所属日期为"+datestr+"的数据！");
						return false;
					}
	        	
	        		
	        	}else{
	        		$.ligerDialog.error("空行不能进行数据修改,请选择当天数据进行修改！");
	        		return false;
	        	}
		...
		function f_onAfterEdit(e)
		{
        	...
        	if (columnname != '' && columnname != null && typeof(this.VERIFIER)!=columnname &&
				...
				pwaterparam = e.value;
				namedisplay = e.column.display.replace("≤","");
				if(parseFloat(pwaterparam) > parseFloat(namedisplay)){
                			
              				pwaterparam = "<span style='color: red;'>"+pwaterparam+"</span>";
              			}
