...
<head><title>高干度锅炉日报维护数据</title>
   ...
   var grid=null;
        $(function () {;
			...
			jQuery.ajax({
				type : 'post',
				url : 'ggdglrd_pageInit.action?arg=rbwh',
      	      ...
      	      var rows = grid.getCheckedRows();
      	          switch (item.icon) {
      	              case "add":
      	                 $.ligerDialog.open({
                    	    			title: "添加高干度锅炉日报数据",
                    	    			url:'addOrUpdateGGDBoilerRPD.jsp',
                    	    			width: 750,
                    	    			height: 500
                    	    		});
      	                  break;
      	              case "modify":
      	                  if (rows.length == 0) { 
      							...
      							var optit='修改高干度锅炉日报数据';
      							$.ligerDialog.open({
                	    			title: "修改高干度锅炉日报数据",
                	    			url:'addOrUpdateGGDBoilerRPD.jsp?optit='+optit,
                	    			//data: {'optit':'修改高干度锅炉日报数据'},
                	    			width: 750,
                	    			height: 500
                	    		});
      	              ...
      	              case "delete":
            	    	 ...
            	    	 var rpdBoilerHighDryId='';
            	    	 $(rows).each(function (){
            	    		 rpdBoilerHighDryId=this.RPD_BOILER_HIGH_DRY_ID+'';
            			});
            	    	 jQuery.ligerDialog.confirm('确定删除吗?', function (confirm) {
     	                      if (confirm){
     	                      	jQuery.ajax({
     							type: "post",
     							url: "ggdglrd_removeGGDRDBoiler.action",
     							data: {'rpdBoilerHighDryId':rpdBoilerHighDryId},
     							timeout: '3000',
