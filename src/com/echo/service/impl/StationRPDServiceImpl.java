...
import com.echo.dao.StationRPDDao;
import com.echo.service.StationRPDService;
import com.echo.util.DateBean;

public class StationRPDServiceImpl implements StationRPDService{
	private StationRPDDao stationRPDDao;

	public void setStationRPDDao(StationRPDDao stationRPDDao) {
		this.stationRPDDao = stationRPDDao;
	}
...
//			etime = nowdate.substring(0, 10) + " 23:59:59";
			etime = nowdate.substring(0, 10);
		}
//		else{
//			etime = etime + " 23:59:59";
//		}
		String formTable = " from PC_RPD_BSTATION_TURNOILD_V  where 1=1  and REPORT_DATE between TO_DATE('"+stime+"','YYYY-MM-DD') and TO_DATE('"+DateBean.getTimeAfterDAYTime(etime)+"','YYYY-MM-DD') ";
...
//		'日产量计算系数--DAILY_OUTPUT_COEFFICIENT
		...
		String[] cloumnsName = {"BLOCKSTATION_NAME","REPORT_DATE","BSTATION_TURNOILDID","BLOCKSTATIONID","BLOCKSTATIONTYPE","CYZ","YXZ","QKMC","EFFLUX_PRES","EFFLUX_TEMP",
				...
				"EFFLUX_PUMP_STATE_2","DAILY_OUTPUT"};
		String[] cloumnsNames2 = {"BLOCKSTATION_NAME","REPORT_DATE","BLOCKSTATIONTYPE","CYZ","YXZ","QKMC","EFFLUX_PRES","EFFLUX_TEMP",
				"EFFLUX_PUMP_FREQ","TANK_LIQUID_LEVEL_1","TANK_LIQUID_LEVEL_2","AMONGST_TANKS_PRES","H2S_CONCENTRATION_1","H2S_CONCENTRATION_2","EFFLUX_PUMP_STATE_1",
				"EFFLUX_PUMP_STATE_2","DAILY_OUTPUT"};
...
//				}
		...
		if ("1".equals(totalNum)) {
			...
			totalNum += formTable;
			total = stationRPDDao.getCounts(totalNum);
		}
		...
		if ("export".equals(totalNumFlag)) {
			lo = stationRPDDao.searchData(thinOilWellRD);
		}
		else {
			...
			if ("export".equals(totalNumFlag)) {
				lo = stationRPDDao.searchData(thinOilWellRD,cloumnsNames2,start,rowsPerpage);
			}else{
				lo = stationRPDDao.searchData(thinOilWellRD,cloumnsName,start,rowsPerpage);
			}
